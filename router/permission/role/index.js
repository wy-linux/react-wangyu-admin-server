...
router.get('/role/info', async(req, res) => {
    ...
    roles = roles.map((role) => role._doc)
    res.send({
...
router.post('/role/update', async(req, res) => {
    await Role.updateOne({_id: req.body._id}, {$set:{
...
router.get('/role/menuInfo/:id', async(req, res) => {
    ...
    menus = menus.map((menu) => {
        if(role.menuSelect.includes(menu._doc._id)) {
            menu._doc.select = true
        }
        return menu._doc
    })
...
router.post('/role/menuInfo/update', async(req, res) => {
    ...
    await Role.updateOne({_id: roleId}, {$set:{
